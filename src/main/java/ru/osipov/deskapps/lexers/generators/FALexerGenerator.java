package ru.osipov.deskapps.lexers.generators;

import ru.osipov.deskapps.vocabularies.Vocabulary;
import ru.osipov.deskapps.fsa.RegexProcessor;
...
import ru.osipov.deskapps.fsa.structures.graphs.Edge;
import ru.osipov.deskapps.fsa.structures.graphs.Elem;
...
public class FALexerGenerator {
    ...
    public static CNFA buildNFA(Vocabulary V){
        Map<String, Word> rules = V.getWords();
        ...
        for(String id : rules.keySet()){
            String pattern = rules.get(id).getPattern();
                ...
                CNFA nfa = RegexProcessor.buildNFA(rpn,parser,idC);
